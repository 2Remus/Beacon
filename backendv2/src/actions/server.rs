...
pub async fn create_server(
    ...
    Ok(format!(
        "Server {} ({}) provisioned at {:?}",
        id, &new_server.version, container_path
    ))
}
...
pub async fn get_servers(data_dir: String) -> napi::Result<Vec<MinecraftServer>> {
    ...
    let content =
        std::fs::read_to_string(&db_path).map_err(|e| Error::from_reason(e.to_string()))?;

    let servers: Vec<MinecraftServer> =
...
async fn ensure_jar_exists(
    ...
    let download_url = match provider {
            ...
            let build_id = resp["builds"]
                ...
                .ok_or_else(|| {
                    napi::Error::from_reason(format!(
                        "No Paper builds found for version {}",
                        version
                    ))
                })?;
        ...
        Provider::Vanilla => {
            ...
            let version_url = manifest["versions"]
                ...
                .ok_or_else(|| {
                    napi::Error::from_reason(format!("Vanilla version {} not found", version))
                })?;
    ...
    use tokio::io::AsyncWriteExt;
    let mut dest = tokio::fs::File::create(&target_path)
        .await
